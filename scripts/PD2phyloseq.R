...
for (str in strains) {
  ...
  #############################################
  #genes list
  gene_annotations_df <- read.csv(paste0("Data/PD_results/Ref_annotations/",str,"-genes.txt"), sep="\t", h= T) 
  
  source_files <- list.files(path = paste0("Data/PD_results/Ref_annotations/"), 
  ...
  annotations_df <- annotations_list %>% 
                    reduce(full_join, by = "gene_callers_id") %>% 
  ...
  metaP_raw <- read.csv(paste0("Data/PD_results/",str,"-Proteins.txt"),
                        sep="\t", h= T)%>% 
